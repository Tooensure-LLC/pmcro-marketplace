...
- The one genuinely reusable idea from that design — keeping external-framework references
  ...
  formalized as a convention rather than a plugin: see `reference-convention.md`.
...
- **Earned Constraints format.** `pmcro-runtime` models a constraint as a JSON entity
  ...
  This repo has a flat markdown list at `docs/governance/earned-constraints.md` with no lifecycle. Don't
  ...
  **Decided 2026-09-16, recorded here instead of silently resolved:** pmcro-runtime's repo/folder
  was deleted (Shawn's call, out of scope going forward) and pmcro-marketplace is now the sole
  product. Its `Domain.Entities.EarnedConstraint` adopts pmcro-runtime's *lifecycle shape*
  (`Candidate → Promoted → Suspended → Retired`, retire-don't-erase) even though pmcro-runtime's
  own code is gone — not rebuilt from memory of that code, just the shape this doc already
  described. It adds a `(SkillId, ScriptId)` scope and a `ConsecutivePassCount`, because a
  same-day session (`maf-production-governance` skill build) independently arrived at needing
  exactly this for Checker-as-approver auto-approval — see "Autonomous-in-the-loop" above: promote
  after N consecutive Checker PASS on the same skill+script pair, demote to Suspended on any single
  HALT/LOOP. This repo's own `docs/governance/earned-constraints.md` stays as the *process* doc (how an
  entry is earned) — it does not need the same fields, since it isn't the runtime data store.
  Still open: the actual N-consecutive-PASS promotion engine and the `AutoApprovalRules` wiring
  are not built — only the data shape is real so far.
...
## No interception point is reachable from the app (surfaced 2026-09-16)

Surfaced while wiring `pmcro-marketplace`'s MAUI UI to the orchestrator. The API
(`src/PMCR.OrchestratorService/Program.cs`) exposes exactly four endpoints: `GET /health`,
`POST /api/cycles`, `GET /api/cycles/{id}`, `GET /api/runtime`. A cycle starts and then runs
Orchestrator → Planner → Maker → Checker → Reflector to completion on its own.

That means **a client can observe a trail but cannot participate in one.** There is no endpoint to
approve a phase, halt a run, or answer a gate. The designed screens assume otherwise — the Active
Trail mock's primary action is "Approve → Check", which no endpoint can satisfy.

The UI was built to what exists rather than to the mock: it follows a run by polling, and "Halt"
stops the client following, explicitly not the orchestrator. The approval affordance was *not*
shipped as a disabled button or a stub, because a control that looks like a gate and isn't one is
worse than no control — it would read as human-in-the-loop governance while the loop ran
unsupervised.

Unresolved, and worth a deliberate Planner phase rather than an ad-hoc endpoint:

- `interception-points.md` names eight MAF-aligned interception points. None of them are
  addressable over HTTP today. Are they meant to be? If the answer is no — if interception is a
  host/CLI-side concern and the service is fire-and-forget by design — then the mock is wrong and
  should be redrawn, not the API extended to chase it.
- If the answer is yes, a phase gate needs a state the trail can *sit in* (awaiting-approval) and
  a resume path, which the current `Status` field and `TrailService` do not model.
- The thesis in "Autonomous-in-the-loop" above depends on this: "human in the loop generates
  training data" requires the human to have somewhere to intervene. Right now they don't.

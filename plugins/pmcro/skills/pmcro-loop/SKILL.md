...
2. **Plan**  
   Act as Planner. Read `docs/governance/earned-constraints.md` first. Write `01-plan.jsonl`. Minimum plan + acceptance criteria.
...
5. **Reflect / Seal**  
   - PASS → write `disposition.json` = ACCEPT, stop. If any loop happened on this trail, Reflector promotes the durable lesson to `docs/governance/earned-constraints.md`.  
   - LOOP and loops < 3 → Reflector notes learning, next cycle on same trail.  
   - HALT or loops ≥ 3 → disposition = HALT, stop for human.

## Laws
See `docs/governance/laws.md` and `AGENTS.md`. Non-negotiable.

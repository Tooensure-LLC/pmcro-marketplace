...
| ~~`EarnedConstraints.{NoHardcodedPaths,NoNewInUoW,TrailSealedImmutable}`~~ | **Reconciled 2026-09-16.** These three hand-seeded constants were removed — they were never earned by a real Reflector promotion, which is what "earned" is supposed to mean. `Domain.Entities.EarnedConstraint` (persisted via `IUnitOfWork`/EF Core, same pattern as `Trail`/`Frame`) is now the single representation. The `pmcro-runtime` repo is no longer part of this reconciliation (Shawn deleted that folder/dropped it from scope 2026-09-16); `extern/pmcro-skills/.pmcro/earned-constraints.md` still defines the *lifecycle* (only a real `LOOP -> ... -> ACCEPT` trail may add an entry) as a process reference, not competing data. | **Declared, correctly empty** — the entity/table exist and build; nothing has written to it because `ReflectorAgent` is still a stub (see below), so there are honestly zero earned constraints yet. |
